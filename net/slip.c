...
size_t al_slip_write(al_slip_t *slip, const void *data, size_t len)
{
    ...
    slip->opt->sl_putc(slip, SLIP_END);
	...
	for (i = 0; i < len; i++) {
		...
		case SLIP_END:
            ...
            slip->opt->sl_putc(slip, SLIP_ESC);
            slip->opt->sl_putc(slip, SLIP_ESC_END);
			break;
		case SLIP_ESC:
			/* need to escape this byte (0xDB -> 0xDB, 0xDD) */
            slip->opt->sl_putc(slip, SLIP_ESC);
            slip->opt->sl_putc(slip, SLIP_ESC_ESC);
			break;
		default:
			/* normal byte - no need for escaping */
            slip->opt->sl_putc(slip, c);
			break;
		}
	}
...
size_t al_slip_recv(al_slip_t *slip)
{
    ...
    while (((c = slip->opt->sl_getc(slip)) != EOF)) {
